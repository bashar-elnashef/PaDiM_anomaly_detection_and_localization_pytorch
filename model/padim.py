...
class PADIM:
    ...
    def fit(self, dataloader: torch.utils.data.DataLoader, extractions: int = 1) -> None:
        ...
        for _ in range(extractions):
            ...
            embedding_vectors = (
                extracted_embedding_vectors
                if embedding_vectors is None
                else torch.cat((embedding_vectors, extracted_embedding_vectors), 0)
            )
        self.mean = torch.mean(embedding_vectors, dim=0)
        # TODO: find optimal rho which is set here to 0.01.  
        cov = util.pytorch_cov(embedding_vectors.permute(1, 0, 2), rowvar=False) \
                + 0.01 * torch.eye(embedding_vectors.shape[2])
        # Run inverse function on splitted tensor to save ram memory
        self.cov_inv = util.split_tensor_and_run_function(func=torch.inverse,
                                                     tensor=cov,
                                                     split_size=1)
    ...
    def predict(self, batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        patch_scores = util.mahalanobis(self.mean, self.cov_inv, embedding_vectors)
...
def get_indices(choose, total, device):
    ...
    return torch.tensor(random.sample(range(total), choose), device=device)
